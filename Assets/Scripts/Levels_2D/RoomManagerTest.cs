...
public class RoomManagerTest : MonoBehaviour
{
    ...
    [FormerlySerializedAs("curreRoomScript")] public RoomTest2 curreRoomTest2Script;
    [FormerlySerializedAs("nextRoomScript")] public RoomTest2 nextRoomTest2Script;
    ...
    private void GenerateRoom(Vector2Int roomIndex)
    {
        ...
        var roomScript = roomObj.GetComponent<RoomTest2>();
        roomScript.RoomIndex = roomIndex;
        roomObjects.Add(roomObj);

        if (roomCount == 1)
        {
            curreRoomTest2Script = roomScript;
            curreRoomTest2Script.isInitialRoom = true;
        }
        else nextRoomTest2Script = roomScript;

        if (roomScript.isInitialRoom == false)
        {
            if (!curreRoomTest2Script.IsLeftBusy)
            {
                curreRoomTest2Script.IsLeftBusy = true;
                curreRoomTest2Script.DisableWallAndDoor(Vector2Int.left);
                nextRoomTest2Script.DisableWallAndDoor(Vector2Int.right);
            ...
            else if (!curreRoomTest2Script.IsTopBusy)
            {
                curreRoomTest2Script.IsTopBusy = true;
                curreRoomTest2Script.DisableWallAndDoor(Vector2Int.up);
                nextRoomTest2Script.DisableWallAndDoor(Vector2Int.down);
                currentRoomIndex = new Vector2Int(initialRoomIndex.x, initialRoomIndex.y + 1);
                var newPosition = GetPositionFromGridIndex(currentRoomIndex);
                roomObj.transform.position = newPosition;
            }
            else if (!curreRoomTest2Script.IsRightBusy)
            {
                curreRoomTest2Script.IsRightBusy = true;
                curreRoomTest2Script.DisableWallAndDoor(Vector2Int.right);
                nextRoomTest2Script.DisableWallAndDoor(Vector2Int.left);
                currentRoomIndex = new Vector2Int(initialRoomIndex.x + 1, initialRoomIndex.y);
                var newPosition = GetPositionFromGridIndex(currentRoomIndex);
                roomObj.transform.position = newPosition;
            }
            else if (!curreRoomTest2Script.IsBottomBusy)
            {
                curreRoomTest2Script.IsBottomBusy = true;
                curreRoomTest2Script.DisableWallAndDoor(Vector2Int.down);
                nextRoomTest2Script.DisableWallAndDoor(Vector2Int.up);
                currentRoomIndex = new Vector2Int(initialRoomIndex.x, initialRoomIndex.y - 1);
                var newPosition = GetPositionFromGridIndex(currentRoomIndex);
                roomObj.transform.position = newPosition;
            }
